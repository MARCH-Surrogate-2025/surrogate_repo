...
kit = SimulationApp({"headless": False})  # headless=False → GUI 실행
...
#sim = SimulationContext(physics_dt=1.0/1000.0)
# URDF import config
...
# steering_drive.GetDampingAttr().Set(0)
...
for frame in range(100000000):
    ...
    log_steering.append(steering_angle)
    kit.update()
# Shutdown and exit
...
plt.grid()
plt.title("Roll")
plt.ylim(-5, 5)  # 여기서 y축 범위 제한
plt.grid()
plt.legend()

# 화면에 띄우는 대신 파일로 저장
plt.savefig("/home/user404/Desktop/plot_roll_data.png")
print("파일 저장 완료")
time.sleep(1)
