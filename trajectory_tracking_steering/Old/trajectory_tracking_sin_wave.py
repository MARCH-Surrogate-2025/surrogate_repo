...
rear_drive.GetStiffnessAttr().Set(0)
rear_drive.GetDampingAttr().Set(150)
rear_drive.GetMaxForceAttr().Set(10)
...
def set_goal(x_now, y_now, yaw_now):
    ...
    total_length = 24
    num_points = 10
    x = i * (total_length / num_points)
    y = 2 * m.sin((x / total_length) * 2 * m.pi)
    slope_angle = 2 * m.atan2(m.pi/total_length*2,1)

    target_x = x * m.cos(slope_angle) + y * m.sin(slope_angle) + 0.407
    #target_x = x * m.cos(slope_angle) + y * m.sin(slope_angle)
    target_y = -x * m.sin(slope_angle) + y * m.cos(slope_angle)
...
def compute_roll_from_local_x():
    ...
    position[0] += 0.407*m.cos(m.radians(yaw_angle))  # x 보정
    position[1] += 0.407*m.sin(m.radians(yaw_angle))
...
for frame in range(100000):
    ...
    rear_drive.GetTargetVelocityAttr().Set(cmd)
